...
public class UDPRT: ScriptableObject 
{
	...
	public delegate void cbEvent(IAsyncResult result);
	...
	public static UDPRT CreateInstance(int Port, cbEvent cb) // RECEVE UDP
	{
		IP = new IPEndPoint(IPAddress.Any, Port);
		udpc = new UdpClient(Port);
		//AC = new AsyncCallback(ReceiveIt);
		//StartUdpReceive();
		udpc.BeginReceive(new AsyncCallback(cb), obj);
		return ScriptableObject.CreateInstance < UDPRT > ();
	}
